...
from threestudio.data.uncond import (
    ...
    RandomCameraInterpolationDataset,
    mix_two
)
...
@register("random-multiview-camera-interpolation-datamodule")
class RandomMultiviewCameraInterpolationDataModule(RandomMultiviewCameraDataModule):
    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = RandomMultiviewCameraIterableInterpolationDataset(self.cfg)
        if stage in [None, "fit", "validate"]:
            self.val_dataset = RandomCameraInterpolationDataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = RandomCameraInterpolationDataset(self.cfg, "test")